...
`plugins/<plugin-name>/` 配下は **source 提供元 repo にのみ存在** する (= 利用側 repo は marketplace + `/plugin install` 経由で Claude Code の plugin loader にロードされた状態を消費し、 repo tree には plugins/ ディレクトリを持たない)。 source 提供元では dogfooding 用 wrapper で当該 dir を `--plugin-dir` 引数として直接 load する経路もあわせて運用してよい (具体経路は各 source 提供元 repo の internal ADR で別途定める)。

#### 利用側 install

利用側は `flame install` (= flame CLI のサブコマンド) を起動すると、 plugin marketplace 登録 (`/plugin marketplace add <owner>/<harness-repo>` 相当) と plugin install (`/plugin install <plugin-name>@<marketplace-name>` 相当) を CLI 内部で自動実行する (= 利用者は Claude Code セッション内で `/plugin` コマンドを手動で叩く必要は無い)。

`.claude-plugin/marketplace.json` を harness の source 提供元 repo に併設する場合は別 marketplace 専用 repo を切らずに同居させる。

#### Version 採番

- `plugin.json.version` を harness ツール (CLI + harness + plugin) 単一 version と同期する
- 利用側は `/plugin update <plugin-name>@<marketplace-name>` で plugin 側 version bump を取り込む
...
jobs:
  check:
    uses: <owner>/<harness-repo>/.github/workflows/wf__check.yaml@vX.Y.Z
```

`@<ref>` は harness ツールの version (git tag) を指す。 利用側は harness ツールと同一 version を tag 指定する。
...
```yaml
# OK
uses: <owner>/<harness-repo>/.github/workflows/wf__check__diff.yaml@<ref>

# NG (caller-relative になり caller repo の同名 workflow を呼びにいく)
uses: ./.github/workflows/wf__check__diff.yaml
```

source 提供元 repo が dogfooding する際は `@<ref>` を当該 repo の default branch (もしくは dev 中の PR branch) に揃える (具体 ref 値は各 source 提供元 repo の internal ADR で定める)。

#### Caller-less な flame CLI 取得経路

実体層 reusable workflow は caller の `actions/checkout` 結果に依存しない (= caller のソースツリー前提を持たない) ように構成する。 harness CLI install のために `actions/checkout: <owner>/<harness-repo>` を **別 path** (`path: .flame-tool`) に追加し、 source 提供元 repo が提供する CLI install スクリプトを実行する。

```yaml
- name: Checkout harness tool
  uses: actions/checkout@<sha>
  with:
    repository: <owner>/<harness-repo>
    ref: <ref>
    path: .flame-tool
- name: Install flame CLI
  ...
  run: |
    bash .flame-tool/<install-script-path>
    echo "$HOME/.local/bin" >> "$GITHUB_PATH"
```

CLI install スクリプトの具体パスは各 source 提供元 repo の internal ADR で定める。 flame CLI install 自体は GitHub Release asset 取得 ([FLM_FEA_0004](FLM_FEA_0004__release_policy.md)) のため、 caller 側 repo の checkout 内容には依存しない。
...
source 提供元 repo は自身の harness を上記 3 チャネルすべてで利用側と同じ経路で参照する (= source 提供元 repo の dev 経路と利用側経路を二重化しない)。

- チャネル A (plugin): source 提供元 repo は `plugins/<plugin-name>/` を SoT として持ち、 dev 環境で plugin を有効化する経路 (marketplace 登録 + install もしくは `--plugin-dir <plugin-dir>` での ad-hoc ロード) を持つ
- チャネル B (reusable WF): source 提供元 repo の install 済 `flame-trg__*.yaml` も自身の `uses: <owner>/<repo>/.github/workflows/<f>.yaml@<ref>` で外部参照する (`<ref>` は当該 repo の default branch / dev 中の PR branch)
- チャネル C (vendor): `vendor/flame/` を SoT とし、 install path は当該 repo root の各位置。 source 提供元 repo の開発者が harness を変更する際は vendor 側 (または overlay 側) を編集し、 `flame install` で repo の install path に同期する。 `flame.lock` は source 提供元 repo でも生成・追跡する

各 source 提供元 repo の具体経路 (plugin 名 / source identity / dev wrapper パス等) は当該 repo の internal ADR で別途定める。
...
2. **`flame install` 実行** — `flame.yaml` (= 不変情報 = `source` / `version`) を repo root に手動で配置した状態で repo root を cwd として `flame install` を実行する (cwd 固定探索のため別 dir からは実行しない)。 CLI が以下を一括で実行する:
   - **チャネル A (plugin)**: Claude Code plugin marketplace の登録 (`/plugin marketplace add <owner>/<harness-repo>` 相当) と plugin install (`/plugin install <plugin-name>@<marketplace-name>` 相当) の自動実行
   - **チャネル B (reusable workflow)**: install 結果の `.github/workflows/flame-trg__*.yaml` で `uses: <owner>/<harness-repo>/.github/workflows/<f>.yaml@<ref>` を解決させる経路の整備 (= ref 切り替えは副ファイル overlay 経由)
...
- **source 提供元 repo は plugin / reusable WF を local path 参照する (dogfooding 経路を二重化)**: source 提供元 repo 内では `--plugin-dir <plugin-dir>` や `uses: ./...` を使い、 利用側のみ外部参照という構成。 実装は単純だが source 提供元 repo 内の参照経路と利用側経路が乖離するため、 利用側で発生する不具合 (例: reusable WF の uses 連鎖が caller-relative に解決される問題) を source 提供元 repo 開発時に再現できない。 source 提供元 repo 自身も外部参照経路で動かすことで実態と一致させる方を採用した
- **reusable wf 内部で harness CLI を caller checkout 経由で取得する**: 現行 (本 ADR 改訂前) は caller の install スクリプトを呼ぶ前提。 caller 側 repo に harness source が無い場合に動作しない。 `actions/checkout: <owner>/<harness-repo>` を別 path に追加して install スクリプトを直接実行することで、 caller の作業ツリーを汚染せず harness CLI を取得する方を採用した
