#!/usr/bin/env bash
# .github/workflows/wf__label__path.yaml (実体層、 leaf = 合成先を持たず
# inline で `flame ci label path-base` を呼ぶ) の dispatch 正しさを検証する
# (FLM_ENG_0003 §test)。
...
target="$repo_root/.github/workflows/wf__label__path.yaml"
